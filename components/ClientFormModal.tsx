...
export const ClientFormModal: React.FC<ClientFormModalProps> = ({ isOpen, onClose, onSave, initialData, currentUser }) => {
    ...
    const [metaAccountName, setMetaAccountName] = useState<string | undefined>(undefined);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        let logoUrl = initialData?.logo || `https://avatar.vercel.sh/${name.trim()}.png?text=${name.trim().charAt(0)}`;
        if (logoFile) {
        ...
        const clientData: Client = {
            ...
            metaAccountName: initialData?.metaAccountName,
        };
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
            <div className="bg-brand-surface rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-lg relative">
                ...
                <form onSubmit={handleSave} className="space-y-4">
                    ...
                    {metaAccountName && (
                        <div>
                            <label htmlFor="metaAccountName" className="block text-sm font-medium text-brand-text-secondary mb-1">Nombre de Cuenta de Meta Vinculado</label>
                            <input
                                id="metaAccountName"
                                type="text"
                                value={metaAccountName}
                                className="w-full bg-brand-bg border border-brand-border text-brand-text-secondary rounded-md p-2 focus:ring-brand-primary focus:border-brand-primary cursor-not-allowed"
                                readOnly
                            />
                        </div>
                    )}
