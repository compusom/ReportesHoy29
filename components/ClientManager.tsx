...
const ClientCard: React.FC<{ client: Client, analysisCount: number, onEdit: () => void, onDelete: () => void, isAdmin: boolean, currentUser: User }> = ({ client, analysisCount, onEdit, onDelete, isAdmin, currentUser }) => {
    return (
        <li className="bg-brand-border/50 p-4 rounded-md flex items-center gap-4 transition-all hover:shadow-lg hover:bg-brand-border/70 relative">
            ...
            {isAdmin && client.userId !== currentUser.id && (
                <span className="text-[10px] text-brand-text-secondary bg-brand-border px-1.5 py-0.5 rounded-full absolute -top-2 -right-2" title={`Asignado a ID: ${client.userId}`}>
                    {client.userId.substring(0, 4)}...
                </span>
            )}
        </li>
    );
